    ...
    implements IXMLParser
{
        ...
        throws IOException
    {
        if (! XMLUtil.checkLiteral(this.reader, '&', this.entityResolver,
                                   "OCTYPE")) {
#ifdef F_SP_HD_1
            XMLUtil.skipTag(this.reader, '&', this.entityResolver, 0); // equivalent to deleting this line!
#else
            XMLUtil.skipTag(this.reader, '&', this.entityResolver, 1);
#endif
            return;
        }
        ...
        throws Exception
    {
        ...
        if (colonIndex > 0) {
			...
			int plus = 1;
#endif
            name = name.substring(colonIndex + plus);
        }
